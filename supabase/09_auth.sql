--------------------------------------------------------------------------------
-- 09_auth.sql — Signup verification codes + verify RPC
--------------------------------------------------------------------------------
...
CREATE INDEX IF NOT EXISTS svc_email_idx ON signup_verification_codes(email);
CREATE INDEX IF NOT EXISTS svc_expires_idx ON signup_verification_codes(expires_at);

ALTER TABLE signup_verification_codes ENABLE ROW LEVEL SECURITY;

-- Allow anonymous insert (frontend stores codes before user exists)
CREATE POLICY "Allow anon insert signup codes"
  ON signup_verification_codes FOR INSERT TO anon
  WITH CHECK (true);

-- RPC: Verify code without exposing it
