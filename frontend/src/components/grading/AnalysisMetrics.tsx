  ...
  return (
    <div className="space-y-6">
      ...
      <Card variant="glass" className="border-white/50">
        <h4 className="text-[10px] font-bold text-neutral-400 uppercase tracking-widest mb-6 border-b border-neutral-100 pb-3">Proficiency Breakdown</h4>
            ...
            return (
              <div key={key} className="flex items-center gap-3 p-3 bg-white/40 border border-white/60 rounded-2xl shadow-sm hover:bg-white/60 transition-colors group">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm font-bold text-neutral-900">{Math.round(score)}<span className="text-[10px] text-neutral-400 ml-0.5">%</span></p>
                </div>
              </div>
            );
          })}
      ...
      {Object.keys(groupedGrammarErrors).length > 0 && (
        <Card variant="glass" className="border-error-100/30 overflow-hidden">
          <div className="flex items-center justify-between mb-5 border-b border-neutral-100 pb-4">
             ...
             <Badge variant="error" size="sm" className="rounded-lg px-2.5 py-1 font-bold text-[10px]">{analysis.detailed_analysis?.grammar?.error_count || 0} Anomalies</Badge>
          </div>
              ...
              return (
                <div key={category} className="bg-white/40 p-4 rounded-2xl border border-white shadow-sm hover:border-primary-100 transition-all duration-300">
                  <div className="flex items-center justify-between mb-3">
                    <span className={`${categoryColor.bg} ${categoryColor.text} px-3 py-1 text-[10px] font-bold uppercase tracking-widest rounded-lg border ${categoryColor.border}`}>
                      {category}
                    </span>
                  </div>
                  <ul className="space-y-2">
                    {errorGroups.slice(0, 4).map((errorGroup, idx) => (
                      ...
                      >
                        ...
                        {errorGroup.count > 1 && (
                          <Badge variant="neutral" size="sm" className="rounded-md px-1.5 py-0.5 text-[9px] bg-neutral-100 border-none font-bold">×{errorGroup.count}</Badge>
                        )}
                      </li>
                    ))}
                  </ul>
      ...
      {analysis.detailed_analysis?.readability && (
        <Card variant="glass" className="border-success-100/30">
          <h4 className="text-[10px] font-bold text-neutral-400 uppercase tracking-widest mb-5">Linguistic Readability</h4>
          <div className="grid grid-cols-2 gap-4">
