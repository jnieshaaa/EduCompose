...
const EssayCard: React.FC<Props> = ({ essay }) => {
  return (
    <div className='p-4 border rounded-rd shadow-sm mb-4'>
      ...
      <p className='text-sm text-neutral-900'>By: {essay.student_id}</p>
      <p className='mt-2'>{essay.content.substring(0, 100)}...</p>

      <div className='mt-3 text-sm'>
        <span className='mr-4'>Grammar: {essay.grammar_score}/5</span>
        <span>Readability: {essay.readability_score}/5</span>
      </div>
    </div>
  );
};
