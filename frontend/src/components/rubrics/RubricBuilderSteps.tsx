...
export function RubricDetailsForm({ formData, onFormChange, onContinue, onCancel }: RubricDetailsFormProps) {
  const gradingIntensities = ['Easy', 'Normal', 'Strict'] as const;

  // Validation: Continue button is disabled if name or program is empty
  const isFormValid = formData.name.trim() !== '' && formData.program !== '';
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-6 border p-4 rounded-lg">
        ...
        <div>
          ...
          <p className="text-sm text-neutral-500 mb-2">Give your rubric a descriptive name</p>
          <Input 
            id="rubric-name" 
        ...
        <div>
          ...
          <p className="text-sm text-neutral-500 mb-2">Control how strict or lenient the AI grading should be.</p>
          <div className="flex gap-2">
            {gradingIntensities.map(intensity => (
              <Button 
                key={intensity}
                size="sm"
                variant={formData.gradingIntensity === intensity ? 'primary' : 'outline'}
                className={formData.gradingIntensity === intensity ? 'bg-primary text-white' : ''}
          ...
          <div className="flex gap-2 flex-wrap">
            {initialProgramsData.map(program => (
              <Button 
                key={program.id}
                size="sm" 
                variant={formData.program === program.name ? 'primary' : 'outline'} 
                className={formData.program === program.name ? 'bg-primary text-white hover:bg-primary-300' : ''}
                onClick={() => onFormChange({ program: program.name })}
              >
                {program.name}
              </Button>
            ))}
...
export function RubricCriteriaEditor({ formData, onFormChange, onSave, onBack }: RubricCriteriaEditorProps) {
    ...
    onFormChange({
      criteria: criteriaList.map(criteria => {
        if (criteria.id === criteriaId) {
          const newScoreId = criteria.scores.length > 0 ? Math.max(...criteria.scores.map(s => s.id)) + 1 : 1;
          const newScore: ScoreLevel = { id: newScoreId, title: 'New Level', points: 0, description: '' };
  ...
  const handleDeleteScoreLevel = (criteriaId: number, scoreId: number) => {
    onFormChange({
      criteria: criteriaList.map(criteria => {
        if (criteria.id === criteriaId) {
          return { ...criteria, scores: criteria.scores.filter(s => s.id !== scoreId) };
        }
        return criteria;
      })
    });
  ...
  const handleScoreChange = (criteriaId: number, scoreId: number, field: keyof ScoreLevel, value: string | number) => {
    onFormChange({
      criteria: criteriaList.map(criteria => {
        if (criteria.id === criteriaId) {
          return {
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-2 border-b pb-4">
        ...
        <div className="flex flex-wrap gap-4 text-sm text-neutral-600">
          <span><strong>Name:</strong> {formData.name || 'Untitled'}</span>
          <span><strong>Program:</strong> {formData.program || 'Not set'}</span>
          <span><strong>Intensity:</strong> {formData.gradingIntensity}</span>
        </div>
      </div>
            ...
            <div key={criteria.id} className="border p-4 rounded-lg space-y-4">
              ...
              <div className="flex items-center gap-2 border-b pb-2">
                <Input
                  ...
                  onChange={(value) => handleCriteriaTitleChange(criteria.id, value)}
                ...
                {criteria.scores.map((score) => (
                  <div key={score.id} className="w-64 flex-shrink-0"> 
                    <div className="flex items-center gap-2 mb-2">
                      <Input 
                        value={score.title} 
                        onChange={(value) => handleScoreChange(criteria.id, score.id, 'title', value)}
                        className="font-medium p-2 text-center flex-grow" 
                      />
                      <Input 
                        type="number" 
                        value={String(score.points)} 
                        onChange={(value) => handleScoreChange(criteria.id, score.id, 'points', parseInt(value) || 0)}
                    ...
                    <textarea
                      ...
                      onChange={(e) => handleScoreChange(criteria.id, score.id, 'description', e.target.value)}
                    />
                  </div>
                ))}
