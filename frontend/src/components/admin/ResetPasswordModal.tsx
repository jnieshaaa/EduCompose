      ...
      >
        <div className="px-8 py-6 border-b border-neutral-100 bg-neutral-50/50 flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-semibold text-neutral-900 tracking-tight leading-tight">Reset Password</h2>
              <p className="text-[10px] font-medium text-neutral-400 uppercase tracking-[0.2em] mt-0.5">Update Password</p>
            </div>
          </div>
        ...
        <div className="p-8 space-y-6">
          <div className="flex items-start gap-3 p-4 bg-amber-50 rounded-2xl border border-amber-100">
             ...
             <div>
                <p className="text-[11px] font-medium text-amber-900 uppercase tracking-widest leading-none mb-1">Account</p>
                <p className="text-xs font-medium text-amber-700 font-mono">{user.email}</p>
             </div>
          </div>

          <AnimatePresence>
            {error && (
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="p-3.5 rounded-2xl bg-red-50 border border-red-100 text-red-700 text-[10px] font-medium uppercase tracking-wider">
                {error}
              </motion.div>
            )}
            {success && (
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="p-3.5 rounded-2xl bg-emerald-50 border border-emerald-100 text-emerald-700 text-[10px] font-medium uppercase tracking-wider">
                {success}
              </motion.div>
            )}
          </AnimatePresence>
          ...
          <form id="reset-secret-form" onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="flex gap-3 pt-4">
              <Button
                ...
                onClick={onClose}
                disabled={isLoading}
                className="flex-1 rounded-xl border-neutral-200 h-10 text-[10px] uppercase font-medium tracking-widest text-neutral-500"
              >
                Cancel
              </Button>
              <Button 
                ...
                disabled={isLoading}
                className="flex-[2] rounded-xl bg-amber-500 text-white shadow-lg shadow-amber-200 h-10 font-medium uppercase text-[10px] tracking-widest"
              >
