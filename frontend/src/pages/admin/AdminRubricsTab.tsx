...
export function AdminRubricsTab() {
  ...
  const loadPlatformRubrics = async () => {
    try {
      ...
      showNotification('error', "Failed to load platform primitives.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreatePlatformRubric = async (rubricData: {
    ...
    try {
      ...
      showNotification('success', "Platform protocol synchronized.");
      await loadPlatformRubrics();
      setShowCreateModal(false);
    } catch (err: any) {
      showNotification('error', "Protocol injection failure.");
    }
  };

  const handleDeletePlatformRubric = async (rubricId: number) => {
    if (!confirm("Initialize protocol decommissioning? This action is irreversible.")) {
      return;
    }
    ...
    try {
      ...
      showNotification('success', "Protocol record decommissioned.");
      await loadPlatformRubrics();
    } catch (err: any) {
      showNotification('error', "Decommissioning failure.");
    }
  };
  ...
  return (
    <div className="space-y-10 pb-20">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-8">
        <div>
          <h1 className="text-3xl font-black text-neutral-900 tracking-tight">Platform Rubrics</h1>
          <p className="text-[10px] font-black text-neutral-400 uppercase tracking-[0.2em] mt-1 flex items-center gap-2">
            ...
            Standardized Instructional Protocols
          </p>
        </div>
        <Button
        ...
        >
          ...
          <span className="text-xs font-black uppercase tracking-widest">Inject Protocol</span>
        </Button>
      </div>
      ...
      <div className="bg-white p-6 rounded-[2.5rem] border border-neutral-100 shadow-sm flex flex-col md:flex-row gap-6">
        ...
        <div className="hidden lg:flex items-center gap-4 px-6 border-l border-neutral-100">
           <div className="text-right">
              <p className="text-[10px] font-black text-neutral-400 uppercase tracking-widest">Active Primitives</p>
              <p className="text-sm font-black text-neutral-900">{platformRubrics.length} Protocols</p>
           </div>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-40 bg-white rounded-[3rem] border border-neutral-100 shadow-sm">
          ...
          <p className="text-[10px] font-black uppercase tracking-[0.3em] text-neutral-400 animate-pulse">Querying Platform Protocols</p>
        </div>
        ...
        <div className="bg-white p-20 rounded-[3.5rem] border border-neutral-100 text-center flex flex-col items-center shadow-sm">
          ...
          <h3 className="text-xl font-black text-neutral-900 tracking-tight">Empty Registry</h3>
          <p className="text-[10px] font-black text-neutral-400 uppercase tracking-[0.2em] mt-2 max-w-sm mx-auto">
            No platform protocols have been injected into the current search window.
          </p>
          <Button
            onClick={() => setShowCreateModal(true)}
            className="mt-8 rounded-xl bg-primary text-white text-[10px] font-black uppercase tracking-widest px-6 h-10"
          >
              ...
              <div className="p-8 space-y-6 flex-1">
                ...
                <div className="pt-4 border-t border-neutral-50 flex items-center justify-between">
                   <div className="flex items-center gap-2">
                      ...
                      <span className="text-[9px] font-black uppercase tracking-widest text-neutral-400">Platform Standard</span>
                   </div>
                   {rubric.grading_intensity && (
                      <span className="text-[9px] font-black text-primary bg-primary/5 px-2.5 py-1 rounded-md uppercase tracking-tighter border border-primary/10">
...
function PlatformRubricModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!name.trim()) {
      setError("Protocol identification required.");
      return;
    }
    ...
    >
      ...
      >
        ...
        <div className="relative z-10 space-y-8">
          <div className="space-y-2">
            <h2 className="text-2xl font-black text-neutral-900 tracking-tight">Inject Platform Protocol</h2>
            <p className="text-[10px] font-black text-neutral-400 uppercase tracking-widest opacity-80">Protocol Genesis & Schema Definition</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <textarea
                ...
                className="w-full h-32 px-5 py-4 bg-neutral-50 border border-neutral-100 rounded-2xl text-sm font-black focus:ring-4 focus:ring-primary/5 focus:bg-white focus:border-primary transition-all outline-none resize-none"
                rows={3}
              />
            </div>
            {error && (
              <div className="p-4 rounded-xl bg-red-50 border border-red-100 text-red-700 text-[10px] font-black uppercase tracking-widest flex items-center gap-3">
                <Trash2 size={12} />
                {error}
              </div>
            )}
            <div className="flex justify-end gap-3 pt-4">
              <Button type="button" variant="ghost" onClick={onClose} className="rounded-xl h-12 text-[10px] font-black uppercase tracking-widest px-6 hover:bg-neutral-50">
                Abort
              </Button>
              <Button type="submit" variant="primary" disabled={isLoading} className="rounded-xl h-12 text-[10px] font-black uppercase tracking-widest px-8 shadow-xl shadow-primary/20">
                {isLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : "Deploy Protocol"}
              </Button>
            </div>
          </form>
        </div>
