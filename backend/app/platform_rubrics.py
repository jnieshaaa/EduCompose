"""
Hardcoded platform rubrics data
These match the frontend rubrics defined in frontend/src/components/rubrics/types.ts
Used as fallback when database is unavailable
"""
from typing import Dict, List, Any, Optional

PLATFORM_RUBRICS: List[Dict[str, Any]] = [
    {
        "id": 1,
    ...
    },
    {
        ...
        "type": "Professional",
        "programs": [],
        "grading_intensity": "Professional",
        "criteria": [
            {
                "id": 1,
...
def get_platform_rubric_by_id(rubric_id: int) -> Optional[Dict[str, Any]]:
    """
    Get a platform rubric by its ID from hardcoded data
    
    Args:
        rubric_id: The numeric ID of the rubric (e.g., 1, 2, 3, 4)
        
    Returns:
        Rubric data dictionary or None if not found
    """
    for rubric in PLATFORM_RUBRICS:
        if rubric["id"] == rubric_id:
            return rubric.copy()  # Return a copy to avoid modifying the original
    return None
